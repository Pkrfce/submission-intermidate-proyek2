...
export default function AddStoryPage() {
  ...
  mainContent.innerHTML = `
    ...
    <section class="form-panel" aria-label="Pilih lokasi di peta">
      <div id="locationMap" class="map-container"></div>
      <p class="help-text">Klik pada peta untuk memilih koordinat lokasi cerita.</p>
    </section>

    <section class="form-panel" aria-label="Form tambah cerita">
      <form id="storyForm" aria-describedby="formHelp" novalidate>
        ...
        <div class="form-field">
          <label for="photoUpload">Foto (unggah berkas atau gunakan kamera)</label>
        ...
        <div class="help-text" id="formHelp">Koordinat akan otomatis terisi saat Anda mengklik peta.</div>

        <button class="btn primary" type="submit">Simpan Cerita</button>
      </form>
  ...
  mapInstance.on("click", (event) => {
    ...
    mainContent.querySelector("#latitudeInput").value = latitude;
    mainContent.querySelector("#longitudeInput").value = longitude;
  ...
  async function initializeCamera() {
    ...
    try {
      ...
      cameraElements.status.textContent = "Kamera aktif. Klik 'Ambil Foto' untuk mengambil gambar.";
      
      cameraElements.preview.onplaying = () => {
        cameraElements.closeButton.disabled = false;
      };
    } catch (error) {
      cameraElements.status.textContent = "Tidak dapat mengakses kamera: " + error.message;
    }
  }
  ...
  async function captureImage() {
    ...
    context.drawImage(cameraElements.preview, 0, 0, cameraElements.canvas.width, cameraElements.canvas.height);
    ...
    cameraElements.status.textContent = "Foto berhasil diambil dan siap diunggah.";
  }
  ...
  window.addEventListener("hashchange", terminateCamera);
  window.addEventListener("beforeunload", terminateCamera);
  ...
  storyForm.addEventListener("submit", async (event) => {
    ...
    const descriptionText = mainContent.querySelector("#storyDescription").value.trim();
    const latitudeValue = parseFloat(mainContent.querySelector("#latitudeInput").value);
    const longitudeValue = parseFloat(mainContent.querySelector("#longitudeInput").value);
    const imageFile = cameraElements.fileInput.files[0] || capturedImageBlob;

    if (!imageFile) {
      cameraElements.status.textContent = "Silakan pilih gambar atau ambil foto dengan kamera.";
      return;
    }
    
    if (Number.isNaN(latitudeValue) || Number.isNaN(longitudeValue)) {
      formMessage.textContent = "Koordinat belum valid. Klik pada peta untuk memilih lokasi.";
      return;
    }
    ...
    try {
      ...
      console.error(error);
      formMessage.textContent = error.message || "Terjadi kesalahan saat menyimpan cerita.";
    }
  });
